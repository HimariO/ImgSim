...
class _FunctionCorrelationTranspose(torch.autograd.Function):
    ...
    def forward(self, input, second):
        rbot0 = second.new_zeros([second.size(0), second.size(2) + 8, second.size(3) + 8, second.size(1)])
        rbot1 = second.new_zeros([second.size(0), second.size(2) + 8, second.size(3) + 8, second.size(1)])
        ...
        if second.is_cuda == True:
            if gradInput is not None or gradSecond is not None:
                ...
                cupy_launch('kernel_Correlation_rearrange', cupy_kernel('kernel_Correlation_rearrange', {
                    ...
                    grid=tuple([int((n + 16 - 1) / 16), gradOutput.size(1), gradOutput.size(0)]),
                    block=tuple([16, 1, 1]),
            ...
            if gradInput is not None:
                ...
                cupy_launch('kernel_Correlation_updateOutput', cupy_kernel('kernel_Correlation_updateOutput', {
                    ...
                    grid=tuple([gradInput.size(3), gradInput.size(2), gradInput.size(0)]),
                    block=tuple([32, 1, 1]),
            ...
            if gradSecond is not None:
                for intSample in range(second.size(0)):
                    ...
                    cupy_launch('kernel_Correlation_updateGradSecond',
                        ...
                        grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
                        block=tuple([512, 1, 1]),
                        args=[n, intSample, rbot0.data_ptr(), rbot1.data_ptr(), input.data_ptr(), None,
                              gradSecond.data_ptr()],
                        stream=Stream
                    )
