...
from dense_match.GOCor.utils_flow.flow_and_mapping_operations import convert_mapping_to_flow, convert_flow_to_mapping
from dense_match.GOCor.local_correlation import correlation
    ...
    def forward(self, im_target, im_source, im_target_256, im_source_256, im_target_pyr=None, im_source_pyr=None,
        ...
        if 'GOCor' in self.params.local_corr_type:
            if self.params.same_local_corr_at_all_levels:
                _corr3 = self.local_corr(c13, warp3)
            else:

                _corr3 = self.local_corr_3(c13, warp3)
        else:
            # feature correlation layer
            _corr3 = correlation.FunctionCorrelation(c13, warp3)
        _corr3 = self.leakyRELU(_corr3)

        if self.params.decoder_inputs == 'corr_flow_feat':
            corr3 = torch.cat((_corr3, up_flow4), 1)
        ...
        if self.iterative_refinement:
            ...
            if nbr_extra_layers == 0:
                ...
                for n in range(nbr_extra_layers):
                    ...
                    if 'GOCor' in self.params.local_corr_type:
                        if self.params.same_local_corr_at_all_levels:
                            _corr3 = self.local_corr(c13_bis, warp3)
                        else:

                            _corr3 = self.local_corr_3(c13_bis, warp3)
                    else:
                        # feature correlation layer
                        _corr3 = correlation.FunctionCorrelation(c13_bis, warp3)
                    _corr3 = self.leakyRELU(_corr3)

                    if self.params.decoder_inputs == 'corr_flow_feat':
                        corr3 = torch.cat((_corr3, up_flow3), 1)
                    elif self.params.decoder_inputs == 'corr':
                        corr3 = _corr3
                    elif self.params.decoder_inputs == 'corr_flow':
                        corr3 = torch.cat((_corr3, up_flow3), 1)
        ...
        if 'GOCor' in self.params.local_corr_type:
            if self.params.same_local_corr_at_all_levels:
                _corr2 = self.local_corr(c12, warp2)
            else:

                _corr2 = self.local_corr_2(c12, warp2)
        else:
            # feature correlation layer
            _corr2 = correlation.FunctionCorrelation(c12, warp2)
        _corr2 = self.leakyRELU(_corr2)
        if self.params.decoder_inputs == 'corr_flow_feat':
            corr2 = torch.cat((_corr2, up_flow3), 1)
        elif self.params.decoder_inputs == 'corr':
            corr2 = _corr2
        elif self.params.decoder_inputs == 'corr_flow':
            corr2 = torch.cat((_corr2, up_flow3), 1)
        ...
        if 'GOCor' in self.params.local_corr_type:
            if self.params.same_local_corr_at_all_levels:
                _corr1 = self.local_corr(c11, warp1)
            else:

                _corr1 = self.local_corr_1(c11, warp1)
        else:
            # feature correlation layer
            _corr1 = correlation.FunctionCorrelation(c11, warp1)
        _corr1 = self.leakyRELU(_corr1)

        if self.params.decoder_inputs == 'corr_flow_feat':
            corr1 = torch.cat((_corr1, up_flow2, up_feat2), 1)
        elif self.params.decoder_inputs == 'corr':
            corr1 = _corr1
        elif self.params.decoder_inputs == 'corr_flow':
            corr1 = torch.cat((_corr1, up_flow2), 1)
        else:
            raise ValueError('Wrong input decoder, you chose {}'.format(self.params.decoder_inputs))
